...
import {
  Search, Clock, MapPin, PackageCheck, ShieldCheck, Globe2, Timer,
  MessageCircle, Plane, Mail, Facebook, Twitter, Linkedin, Instagram,
  ArrowRight, TrendingUp, Headphones, Award, CheckCircle2,
} from 'lucide-react';
import type { SiteConfig } from '@/lib/site-config';
import { getIcon } from '@/lib/icon-map';

const WHY_CHOOSE = [
  { icon: ShieldCheck, title: 'Secure & Insured', desc: 'End-to-end protection with full-value insurance on every shipment.', color: 'text-indigo-600 dark:text-indigo-400', bg: 'bg-indigo-100 dark:bg-indigo-900/40' },
  { icon: Globe2,      title: 'Worldwide Reach', desc: 'Connect to 220+ countries through our global partner network.',     color: 'text-cyan-600 dark:text-cyan-400',     bg: 'bg-cyan-100 dark:bg-cyan-900/40' },
  { icon: Timer,       title: 'Time-Definite',   desc: 'Guaranteed delivery windows you can plan your business around.',     color: 'text-amber-600 dark:text-amber-400',   bg: 'bg-amber-100 dark:bg-amber-900/40' },
  { icon: Headphones,  title: '24/7 Support',    desc: 'Real humans available around the clock to answer your questions.',  color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-100 dark:bg-emerald-900/40' },
];
...
const SERVICE_GRADIENTS = [
  { gradient: 'from-indigo-500 to-violet-600', bgGlow: 'bg-indigo-500/10' },
  { gradient: 'from-cyan-500 to-sky-600',      bgGlow: 'bg-cyan-500/10' },
  { gradient: 'from-amber-500 to-orange-500',  bgGlow: 'bg-amber-500/10' },
  { gradient: 'from-emerald-500 to-teal-600',  bgGlow: 'bg-emerald-500/10' },
  { gradient: 'from-rose-500 to-pink-600',     bgGlow: 'bg-rose-500/10' },
  { gradient: 'from-violet-500 to-fuchsia-600', bgGlow: 'bg-violet-500/10' },
];

export default function LandingPage({ siteConfig }: { siteConfig: SiteConfig }) {
  const [trackingInput, setTrackingInput] = useState('');
  const [isTracking, setIsTracking] = useState(false);
  const [wordIndex, setWordIndex] = useState(0);
  const router = useRouter();

  const { hero, contact, announcement, feature_flags: flags, services } = siteConfig;
  const rotatingWords = hero.rotating_words.length > 0 ? hero.rotating_words : ['Logistics'];
          ...
          <div className="grid lg:grid-cols-3 gap-8 lg:gap-5 items-center">
            <div className="order-2 lg:order-1 lg:col-span-2 animate-fade-in-up">
              <div className="flex flex-col space-y-6 lg:px-10">
                <h1 className="font-extrabold text-gray-900 dark:text-white text-3xl md:text-5xl lg:text-6xl leading-[1.1] tracking-tight" data-testid="text-hero-title">
                  {hero.title_prefix}{' '}
                  <span key={wordIndex} className="inline-block text-indigo-600 dark:text-indigo-400 animate-word-rotate">
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  <Link href={hero.primary_cta_href}>
                    <Button size="lg" className="rounded-md bg-indigo-600 hover:bg-indigo-500 text-white px-7 py-6 text-sm tracking-wide font-medium shadow-lg shadow-indigo-500/30 transition-all" data-testid="button-get-started">
                      {hero.primary_cta_label} <ArrowRight className="w-4 h-4 ml-2" />
                    </Button>
                  </Link>
                  <Link href={hero.secondary_cta_href}>
                    <Button size="lg" variant="outline" className="rounded-md border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 hover:border-indigo-500 hover:text-indigo-600 px-7 py-6 text-sm tracking-wide font-medium" data-testid="button-track-shipment">
                ...
                <div className="grid grid-cols-2 gap-6 pt-4 max-w-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-semibold text-gray-900 dark:text-white text-sm">{hero.badge1_title}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">{hero.badge1_label}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-semibold text-gray-900 dark:text-white text-sm">{hero.badge2_title}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">{hero.badge2_label}</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-5 lg:px-20 pt-8 lg:pt-12">
          <div className="grid grid-cols-7 gap-2 items-end">
            <div className="relative col-span-2 lg:col-span-2 flex items-end justify-start">
              ...
              <Image src="/section4-character.svg" alt="Shipnix character" width={96} height={445} className="relative z-10 w-20 sm:w-24 md:w-28 lg:w-auto lg:h-[22rem] h-auto" data-testid="img-free-character" />
            </div>
              ...
              <div className="rounded-2xl rounded-b-none p-[1px] bg-gradient-to-b from-gray-600 to-gray-900">
                <div className="rounded-2xl rounded-b-none p-5 sm:p-6 lg:p-8 bg-gray-900 dark:bg-black space-y-5 lg:space-y-6">
                  <div className="flex flex-col space-y-3 lg:space-y-4">
                    <h2 className="font-bold text-white text-xl sm:text-2xl md:text-3xl lg:text-4xl leading-tight tracking-tight" data-testid="text-free-headline">
                      Shipnix is free to use
                    </h2>
                    <p className="text-gray-300 text-xs sm:text-sm md:text-base leading-relaxed">
                      By leveraging the shipping volume directed to our partner carriers, we offer Shipnix to our users free of charge — empowering businesses like yours to gain a competitive advantage.
                    </p>
                  </div>
                  <div className="flex flex-col items-center space-y-4">
                    <div className="grid w-full grid-cols-1 gap-2 lg:gap-3 rounded-xl border border-gray-700 p-3 sm:p-4 sm:grid-cols-2">
                      {['Unlimited users', 'No monthly cost', 'Unlimited orders', 'No label limits'].map(feature => (
                        <div key={feature} className="flex items-center space-x-2">
                          ...
                          <span className="tracking-wide text-white text-xs sm:text-sm">{feature}</span>
                        </div>
                      ))}
                    </div>
                    {flags.register_enabled && (
                      <Link href="/register">
                        <Button className="rounded-md bg-indigo-500 hover:bg-indigo-400 text-white px-8 py-5 text-sm tracking-wide" data-testid="button-start-shipping">
                          Start shipping
                        </Button>
                      </Link>
                    )}
            ...
            <div className="col-span-2 lg:col-span-2 flex items-end justify-end animate-fade-in-up">
              <Image src="/section4-illustration.svg" alt="Shipnix is free to use" width={456} height={300} className="w-auto h-44 sm:h-56 md:h-64 lg:h-72 max-w-full" data-testid="img-free-illustration" />
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-16">
              {services.slice(0, 6).map((s, i) => {
                ...
                const { gradient, bgGlow } = SERVICE_GRADIENTS[i % SERVICE_GRADIENTS.length];
                return (
                  <div
                    key={`${s.title}-${i}`}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
            <div>
              ...
              <div className="flex gap-2">
                {[{Icon: Facebook, label: 'Facebook'}, {Icon: Twitter, label: 'Twitter'}, {Icon: Linkedin, label: 'LinkedIn'}, {Icon: Instagram, label: 'Instagram'}].map(({ Icon, label }) => (
                  <a key={label} href="#" aria-label={label} className="w-9 h-9 bg-gray-800 hover:bg-indigo-600 rounded-full flex items-center justify-center transition-colors hover:scale-110 transform duration-200">
            ...
            <div>
              ...
              <ul className="space-y-2.5 text-sm">
                ...
                {flags.services_enabled && <li><Link href="/services" className="hover:text-indigo-400 transition-colors">Services</Link></li>}
                {flags.contact_enabled && <li><Link href="/contact" className="hover:text-indigo-400 transition-colors">Contact</Link></li>}
                {flags.faq_enabled && <li><Link href="/faq" className="hover:text-indigo-400 transition-colors">FAQ</Link></li>}
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2.5 text-sm">
                {services.slice(0, 3).map(s => (
                  <li key={s.title}><Link href="/services" className="hover:text-indigo-400 transition-colors">{s.title}</Link></li>
                ))}
                {flags.quote_enabled && <li><Link href="/quote" className="hover:text-indigo-400 transition-colors">Get a Quote</Link></li>}
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2.5 text-sm">
                {flags.tracking_enabled && <li><Link href="/track" className="hover:text-indigo-400 transition-colors">Track Shipment</Link></li>}
                {flags.faq_enabled && <li><Link href="/faq" className="hover:text-indigo-400 transition-colors">Help Center</Link></li>}
                {flags.contact_enabled && <li><Link href="/contact" className="hover:text-indigo-400 transition-colors">Customer Care</Link></li>}
                <li><a href={contact.whatsapp_url} target="_blank" rel="noopener noreferrer" className="hover:text-indigo-400 transition-colors">WhatsApp Us</a></li>
              </ul>
            </div>
          </div>
          ...
          <div className="border-t border-gray-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-gray-400">
            ...
            <div className="flex flex-wrap gap-5">
              ...
              {flags.contact_enabled && <Link href="/contact" className="hover:text-white transition-colors">Contact</Link>}
              {flags.faq_enabled && <Link href="/faq" className="hover:text-white transition-colors">FAQ</Link>}
            </div>
          </div>
        </div>
      </footer>
