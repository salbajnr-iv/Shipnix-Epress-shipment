-- Shipnix-Express — CMS / Site Settings Migration
-- Run this in your Supabase SQL Editor at https://app.supabase.com
-- Idempotent — safe to re-run.
--
-- Adds a single `site_settings` table that backs the admin Customization
-- console: hero copy, contact info, announcement banner, FAQ items, services,
-- pricing config, time slots, and feature flags.
--
-- Public visitors can READ settings (so the marketing site can render them).
-- Only admins (public.is_admin()) can write.
...
-- =====================================================================
-- SEED DEFAULTS (only inserted if the key doesn't exist yet)
-- =====================================================================

INSERT INTO site_settings (key, value) VALUES
('hero', '{
  "title_prefix": "Streamline your shipping with our all-in-one solution for",
  "rotating_words": ["Carrier Integrations", "Real-Time Tracking", "Instant Quotes", "Global Delivery", "Smart Logistics"],
  "subtitle": "A custom-built, cloud-based logistics platform designed to save you time, effort, and money while you grow your business. Handle every shipment with ease using Shipnix.",
...
('announcement', '{
  "enabled": false,
  "message": "Holiday delays in effect — please allow an extra 1–2 days on international shipments.",
  "link": "/contact",
...
('feature_flags', '{
  ...
  "maintenance_message": "Shipnix Express is undergoing scheduled maintenance. We''ll be back shortly."
}'::jsonb),
...
('services', '[
  {"title": "Express Delivery",     "description": "Lightning-fast shipping when every minute counts.", "bullets": ["Next business day", "International express", "Time-definite delivery"], "icon": "Zap",       "enabled": true},
  {"title": "Ground Shipping",      "description": "Cost-effective ground shipping at scale.",          "bullets": ["1-5 business days", "Residential delivery", "Commercial delivery"], "icon": "Truck",     "enabled": true},
  {"title": "Freight & Logistics",  "description": "Less-than-truckload and full freight services.",    "bullets": ["LTL & FTL shipping", "Freight management", "Supply chain solutions"], "icon": "Warehouse", "enabled": true}
]'::jsonb),

('faqs', '[
  {"q": "How do I track my shipment?", "a": "Enter your tracking ID on the Track page or go to /track/[your-tracking-id]. You will see real-time status updates and your full delivery timeline.", "enabled": true},
